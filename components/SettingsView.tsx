...
import { Save, CheckCircle, FileSpreadsheet, Copy, Zap, AlertCircle, CreditCard, Plus, Trash2, Building, User } from 'lucide-react';
...
export const SettingsView: React.FC<SettingsViewProps> = ({ settings, onSave }) => {
  const [formData, setFormData] = useState<AppSettings>({
    ...settings,
    bankAccounts: settings.bankAccounts || [] // Ensure array exists
  });
  ...
  const handleTestConnection = async () => {
    ...
    setTimeout(() => {
        setIsTesting(false);
        alert('Đã gửi yêu cầu test! Hãy kiểm tra Google Sheet của bạn. Nếu thấy dòng mới "TEST-CONNECTION" tức là đã thành công.');
    }, 1000);
  };
...
function doPost(e) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300 pb-24">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-soft border border-slate-50">
        <h3 className="font-bold text-slate-800 mb-4">Thông tin cửa hàng</h3>
        <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1 flex items-center gap-1">
                        <User className="w-4 h-4" /> Tên nhân viên / Thu ngân
                    </label>
                    <input
                        type="text"
                    ...
                    />
                    <p className="text-[10px] text-slate-400 mt-1">Tên này sẽ được lưu vào đơn hàng trên Google Sheet</p>
                </div>
            </div>
