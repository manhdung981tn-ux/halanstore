...
import { Search, Plus, Minus, Trash2, ShoppingCart, CreditCard, CheckCircle, Banknote, Smartphone, Check, Calculator } from 'lucide-react';
...
export const POSView: React.FC<POSViewProps> = ({
  ...
  const changeAmount = Math.max(0, amountGiven - totalAmount);

  // Update default amountGiven when total changes
  useEffect(() => {
    if (amountGiven === 0 || amountGiven < totalAmount) {
       // setAmountGiven(totalAmount); // Optional: Auto-fill
    }
  }, [totalAmount]);

  const handlePay = () => {
    onCheckout(paymentMethod, paymentMethod === 'transfer' ? paymentNote : `Khách đưa: ${formatCurrency(amountGiven)} - Thừa: ${formatCurrency(changeAmount)}`);
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)]">
      ...
      <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
        ...
        {cart.length > 0 && (
          <div className="bg-white border-t md:border-t-0 md:border-l border-slate-200 md:w-96 flex flex-col shadow-[0_-4px_20px_-2px_rgba(0,0,0,0.1)] z-20">
            ...
            <div className="p-4 border-t border-slate-100 bg-white space-y-3">
                ...
                {paymentMethod === 'cash' && (
                  <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-3 animate-in fade-in zoom-in duration-200">
                    <div>
                      <label className="text-[10px] uppercase font-bold text-slate-500 mb-1 block">Khách đưa</label>
                      <input 
                        type="number" 
                        value={amountGiven || ''}
                        onChange={(e) => setAmountGiven(Number(e.target.value))}
                        placeholder="Nhập số tiền..."
                        className="w-full px-3 py-2 rounded-lg border border-slate-300 font-bold text-lg text-slate-800 outline-none focus:border-brand-blue"
                      />
                      {/* Quick Money Buttons */}
                      <div className="flex gap-2 mt-2 overflow-x-auto no-scrollbar pb-1">
                        <button onClick={() => setAmountGiven(totalAmount)} className="px-2 py-1 bg-white border border-slate-200 rounded text-xs font-medium hover:border-brand-blue">Đủ</button>
                        <button onClick={() => setAmountGiven(100000)} className="px-2 py-1 bg-white border border-slate-200 rounded text-xs font-medium hover:border-brand-blue">100k</button>
                        <button onClick={() => setAmountGiven(200000)} className="px-2 py-1 bg-white border border-slate-200 rounded text-xs font-medium hover:border-brand-blue">200k</button>
                        <button onClick={() => setAmountGiven(500000)} className="px-2 py-1 bg-white border border-slate-200 rounded text-xs font-medium hover:border-brand-blue">500k</button>
                      </div>
                    </div>
                    
                    <div className="flex justify-between items-center pt-2 border-t border-slate-200">
                      <span className="text-xs font-bold text-slate-500">Tiền thừa trả khách</span>
                      <span className={`text-lg font-bold ${changeAmount >= 0 ? 'text-brand-blue' : 'text-red-500'}`}>
                ...
                {paymentMethod === 'transfer' && (
                    <div className="animate-in fade-in slide-in-from-top-2 duration-200 space-y-2">
                         ...
                         <input 
                            type="text" 
                            placeholder="Nhập ghi chú (VD: Tên người chuyển)..." 
                ...
                <button 
                    onClick={handlePay}
                    disabled={isCheckoutSuccess || (paymentMethod === 'cash' && amountGiven < totalAmount)}
                    className={`w-full py-3 rounded-xl font-bold text-white shadow-lg shadow-brand-blue/30 flex items-center justify-center gap-2 transition-all active:scale-95 ${
                        isCheckoutSuccess 
                          ? 'bg-green-500' 
                          : (paymentMethod === 'cash' && amountGiven < totalAmount) 
                ...
                >
                    {isCheckoutSuccess ? (
                        ...
                        <>
                            ...
                            {paymentMethod === 'cash' ? 'Hoàn tất thu tiền' : 'Xác nhận chuyển khoản'}
                        </>
                    )}
                </button>
            </div>
        ...
        {cart.length === 0 && (
             <div className="hidden md:flex md:w-96 flex-col items-center justify-center text-slate-400 bg-slate-50 border-l border-slate-200 p-8 text-center">
