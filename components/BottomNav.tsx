...
import { LayoutGrid, Package, Plus, ShoppingCart, Settings } from 'lucide-react';
...
export const BottomNav: React.FC<BottomNavProps> = ({ activeTab, onTabChange, onAddClick, onNewOrderClick, cartItemCount }) => {
  const navItems = [
    ...
    { id: 'quick-order', icon: Plus, label: '', isAction: true }, // Changed ID to quick-order
          ...
          if (item.isAction) {
            return (
              <div key={item.id} className="relative -top-6 px-2">
                <button
                  onClick={onNewOrderClick} // Changed to open Quick Order Modal
